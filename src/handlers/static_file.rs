...
use hyper::header::{CACHE_CONTROL, CONTENT_TYPE, LAST_MODIFIED, HeaderValue};
...
impl StaticFileHandler {
  ...
  pub fn new(
    file_path: String,
    content_type: &str,
    cache_control: &str) -> Result<Self, Box<::std::error::Error>> {
      ...
      .and_then(move |file| {
        file.metadata().
          and_then(move |metadata_result| {
            ...
            ::tokio_io::io::read_to_end(metadata_result.0, buf)
              .and_then(move |read_result| {
